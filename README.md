...
├── src/                        # Training & preprocessing source code
│   ├── preprocessing.py
│   ├── model_rf.py              # Random Forest model (deployed)
│   ├── train_rf.py
│   ├── model.py                 # Keras/NN model (experimental)
│   └── train.py
├── models/                     # Trained artifacts
│   ├── attrition_model_rf.joblib
│   ├── feature_names.json
│   ├── metrics_rf.json          # Deployed RF metrics
│   └── metrics.json             # Experimental NN metrics
├── data/
│   ├── raw/                    # Original dataset (not tracked)
│   └── processed/              # Processed dataset (not tracked)
├── notebooks/
│   └── 01_eda.ipynb            # Exploratory data analysis
├── reports/                     # Generated plots (confusion matrix, ROC, etc.)
├── tests/
│   └── test_api.py
├── .github/workflows/main.yml  # CI: lint, test, Docker build
├── docker-compose.yml          # Orchestrates db + api + ui
├── Dockerfile                  # Backend API image
├── Dockerfile.streamlit        # Frontend UI image
├── QUICKSTART.md               # 1-day execution guide
...
| `POST` | `/predict` | Run inference on employee data and return the attrition probability. |
| `POST` | `/explain` | Run inference and return probability + SHAP contributions. |
...
- 🌐 [Portfolio](https://portofolio-niko-ardyan.vercel.app/)
